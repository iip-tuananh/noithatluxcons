...
class ProductController extends Controller
{
    ...
    public function detail_product($cate,$slug)
    {
        ...
        ])->where(['slug'=>$slug, 'cate_slug'=>$cate, 'status'=>1])->first(['id','name','images','type_cate','category','sku','discount','price','content','size','description','slug','preserve','created_at','cate_slug']);
        $data['productlq'] = Product::where(['category'=>$data['product']->category, 'cate_slug'=>$cate])->get(['id','name','images','discount','price','slug','cate_slug','type_slug','description']);
