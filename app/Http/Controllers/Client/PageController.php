...
class PageController extends Controller
{
    ...
    public function aboutUs(){
        ...
        $data['album'] = array_chunk(Prize::where(['status'=>1])->get(['id','name','image'])->toArray(), 5);
        $data['albumMobile'] = Prize::where(['status'=>1])->get(['id','name','image']);
        $data['aboutUs'] = PageContent::where(['type'=>'ve-chung-toi','language'=>'vi','status'=>1])->first(['id','title','content']);
    ...
    {
        $data['list_service'] = Services::where(['status'=>1])->get();
